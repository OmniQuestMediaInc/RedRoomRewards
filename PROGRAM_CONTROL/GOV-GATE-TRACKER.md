...
**Authority:** RRR-GOV-002 charter (`PROGRAM_CONTROL/DIRECTIVES/QUEUE/RRR-GOV-002.md`)
**Created by:** RRR-GOV-002-A003
...
If a directive is **not** `CEO_GATE: YES` in §6, it does not appear here — it
is governed by the normal auto-merge path and is tracked in §6 itself.

## How this file is maintained

- **On PR open** for a `CEO_GATE: YES` task: the executing agent adds a row
  with `Status: OPEN`, the PR number, and the date opened.
- **On PR merge** (by CEO): the executing agent flips `Status: OPEN` →
  `Status: MERGED`, fills in the merge date, and notes the merge commit SHA
  in the Notes column.
