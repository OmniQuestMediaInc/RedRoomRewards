...
- **PRs are opened `ready-for-review` by default, never as drafts.** Standing CEO authorization extended 2026-04-23 — harness defaults that would mark new PRs as drafts are overridden by this charter. If a harness auto-creates a draft, the agent flips it to ready-for-review in the same session. Drafts are only permitted when the task itself explicitly calls for one (none currently do).  
- **Auto-merge is pre-authorized for every non-FIZ, non-`CEO_GATE: YES` task.** Once CI is green and the PR is ready-for-review, the agent does not wait for, or request, a separate merge approval. FIZ and `CEO_GATE: YES` PRs continue to wait for human merge per §3.5 item 18.  
...
| 2026-04-23 | Charter restored to `PROGRAM_CONTROL/DIRECTIVES/QUEUE/RRR-GOV-002.md` (previously archived under PR #252 / commit 8064abb when `RRR-WORK-001` was briefly ratified as successor — CEO reversed that retirement 2026-04-23). §2.2 amended with two additional DROID MODE bullets: (a) PRs are opened `ready-for-review` by default, never as drafts — overriding any harness default that would create drafts; (b) auto-merge is pre-authorized for every non-FIZ, non-`CEO_GATE: YES` task on CI green, with no separate per-PR merge approval required. These codify the CEO's 2026-04-23 statement *"you should be fully enabled to create PRs as needed and merge them as required."* Placeholder `PROGRAM_CONTROL/DIRECTIVES/QUEUE/RRR-WORK-001.md` removed to restore §8's "charter is the sole queue file" invariant. | Architecture Coordinator (Claude Chat, ARCHITECT MODE — CEO authorization in-session 2026-04-23: *"Anything on your RRR-GOV-002.md file Or resulting from it is already approved, and you should be fully enabled to create PRs as needed and merge them as required. Can we please make this change hardened in directives"*) | Stop harness-default draft state from blocking auto-merge; make the PR/merge pre-approval durable across sessions so no future agent re-asks |
