# BUILD CONTROL - REWARDS AI Development Reference Index

**Authority:** Kevin B. Hartley, CEO — OmniQuest Media Inc. **Initialized:**
2026-04-18 **Source:** CelaDaniel/free-ai-resources-x (MIT license) **Archive
status:** This document IS the extracted archive. Do not re-pull source repo —
treat this as authoritative.

---

## CORE RESOURCES (universal — apply to all OQMInc projects)

### Generative AI and LLMs

- **Microsoft: Generative AI for Beginners** (18-lesson, free)
  https://github.com/microsoft/generative-ai-for-beginners
- **DeepLearning.AI: Generative AI for Everyone**
  https://www.coursera.org/learn/generative-ai-for-everyone
- **Anthropic Claude Documentation**
  https://docs.anthropic.com/en/docs/intro-to-claude
- **Andrej Karpathy: Neural Networks Zero to Hero**
  https://youtube.com/playlist?list=PLAqhIrjkxbuWI23v9cThsA9GvCAUhRvKZ
- **vLLM** (high-throughput production serving) https://docs.vllm.ai/

### AI Agents

- **DeepLearning.AI: AI Agents in LangGraph**
  https://www.deeplearning.ai/short-courses/ai-agents-in-langgraph/
- **Hugging Face: Building AI Agents**
  https://huggingface.co/learn/agents-course/unit0/introduction
- **AutoGen** (Microsoft multi-agent framework)
  https://microsoft.github.io/autogen/
- **CrewAI:** https://docs.crewai.com/
- **MCP Protocol:** https://modelcontextprotocol.io/
- **Anthropic Tool Use**
  https://docs.anthropic.com/en/docs/build-with-claude/tool-use

### Prompt Engineering

- **Anthropic Prompt Engineering Guide** (PRIMARY for all OQMInc projects)
  https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview
- **DAIR.AI Guide:** https://www.promptingguide.ai/
- **Learn Prompting:** https://learnprompting.org/
- **DSPy** (programmatic optimization): https://dspy-docs.vercel.app/

### AI Security

- **OWASP LLM Top 10**
  https://owasp.org/www-project-top-10-for-large-language-model-applications
- **Garak LLM Scanner:** https://github.com/leondz/garak
- **Lakera Prompt Injection Guide**
  https://www.lakera.ai/blog/guide-to-prompt-injection

### MLOps and Production

- **MLflow:** https://mlflow.org/docs/latest/index.html
- **Weights and Biases:** https://www.wandb.courses/courses/mlops-course
- **Full Stack Deep Learning:** https://fullstackdeeplearning.com/

### NLP and Content Processing

- **Hugging Face NLP Course:** https://huggingface.co/learn/nlp-course
- **spaCy:** https://spacy.io/

### Edge AI and Embedded

- **TensorFlow Lite for Microcontrollers**
  https://www.tensorflow.org/lite/microcontrollers
- **Edge Impulse:** https://edgeimpulse.com
- **TinyML Foundation:** https://www.tinyml.org/

### Recommender Systems

- **Google Recommendation Systems Course**
  https://developers.google.com/machine-learning/recommendation
- **LightFM:** https://making.lyst.com/lightfm/docs/home.html

---

## RRR-SPECIFIC AI MAPPINGS

[POPULATE as AI-adjacent directives are identified]

| Directive                            | AI Component | Primary Resources Above |
| ------------------------------------ | ------------ | ----------------------- |
| [Add rows as directives are defined] |              |                         |
