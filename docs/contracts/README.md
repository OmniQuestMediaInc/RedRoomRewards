...
This directory contains canonical data contracts for integrations between
RedRoomRewards and its merchant clients (e.g., ChatNow.Zone).

## Contents

### Schema Definition

- **[xxx-events.schema.json](./xxx-events.schema.json)** - JSON Schema for event
  ingestion
  - Defines structure for `token_purchase`, `membership_purchase`, `adjustment`,
    and `reversal` events
...
- **[examples/token_purchase.json](./examples/token_purchase.json)** - Token
  purchase event example
- **[examples/membership_purchase.json](./examples/membership_purchase.json)** -
  Membership purchase event example
- **[examples/adjustment_cs_award.json](./examples/adjustment_cs_award.json)** -
  Customer service adjustment example

### Documentation

- **[idempotency-and-retries.md](./idempotency-and-retries.md)** - Comprehensive
  guide to:
...
- `source_system`, `merchant_id`, `loyalty_member_id`
- `adjustment_reason`
  (customer_service_award|compensation|correction|bonus|technical_issue|chargeback|fraud)
...
- Different `idempotency_key` + same `transaction_id` + different data = 409
  Conflict
