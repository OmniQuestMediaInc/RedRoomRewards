...
Planned Expansion: RedRoomRewards network plus third-party tenants (target 12 to
...
- Launch the first branded loyalty rewards points program in adult entertainment
  on ChatNow.Zone.
- Increase retention, repeat token-pack purchases, and VIP tier progression.
- Enable model-driven loyalty: models receive monthly point allocations and gift
  points to fans in-stream.
- Provide near real-time balances and deterministic, auditable earn and redeem
  decisions.

### Product scope

- All product types are in scope for eligibility, but earning and redemption
  eligibility is configurable per product, per action, per tier, and per time
  window.
- Launch priority: token pack purchases and points redemption against token pack
  purchases.
...
- Model: receives point allocations and can gift points to users; model does not
  redeem points.
...
Tier policies must be configured via effective-dated tables, no hard-coded caps
or rates.

### Identity linking

- One loyalty account can link multiple ChatNow.Zone usernames.
- Engine must resolve: loyalty_account_id ⇄ {tenant_id, site_username}, role,
  current tier.
...
- Model Allocation Wallet: non-redeemable balance for models intended solely for
  gifting to users.
...
The engine must track points as lots (award batches) to support expiry ordering
and audit.
...
- If clawback of redeemed value fails, negative balances are allowed (see
  Section 7).
...
This ensures micro top-up points from today are spent before micro top-up points
from tomorrow.
...
- Model segment multipliers: example, Pride week model multiplier 1.5 points per
  $1 earned (for next allocation).
...
- Segmentation: tier, role, country, tags, birthday window, other profile
  fields.
...
If multiple promos apply, select the promo that yields the most value to the
user. Tie-break must be deterministic (configured rule, then earliest end time,
then promo id).
...
Micro top-up exists to unblock redemption when the user is within a few points
of a threshold. It is not positioned as savings.
...
- 5000
- 10000 Admin may extend the list.
...
- Micro top-up points expire in 1 year and follow the spend ordering rules
  (earliest expiry then FIFO).
...
- Configure tiers, earn multipliers, redemption caps, and valuation tables with
  effective dates.
- Configure product eligibility and action eligibility for earning and
  redemption.
...
- correlation_id
- idempotency_key
- event_type (earn, redeem_reserve, redeem_commit, redeem_release, reverse,
  gift, allocation, adjustment, micro_topup_purchase)
- wallet_id
- points_delta
- lot_id nullable
- reason_code
...
GET /v1/balance?tenant_id=...&loyalty_account_id=... Returns:
...
POST /v1/checkout/reserve Request:
...
POST /v1/checkout/commit Request:

- tenant_id
- reservation_id
- order_id
- payment_status (success) Response:
- committed_points
- discount_value_usd
...
POST /v1/checkout/release Request:
...
POST /v1/admin/promotions POST /v1/admin/tiers POST /v1/admin/allocations/models
POST /v1/admin/adjustments All require role-based authorization and full audit
metadata.
...
- Lock triggers: multiple IPs redeeming from one account, velocity anomalies,
  chargeback patterns.
...
- Given cap percent for tier, quote returns max redeemable points constrained by
  cap
