...
**Purpose**: Standardize GitHub Copilot behavior across all OmniQuestMedia
repositories  
**Date**: 2025-12-15  
...
This document defines mandatory rules for GitHub Copilot when generating code,
documentation, or pull requests in the RedRoomRewards repository. All
AI-assisted development must follow these instructions without exception.

**Enforcement**: Copilot MUST reference this file before taking any action. All
outputs must respect security, audit, and minimal-change principles defined
herein.
...
**Rule**: The repository content is the single source of truth. External
assumptions or patterns are prohibited.
...
- Before suggesting changes: Review `/docs/DECISIONS.md` and
...
**Rule**: Security and integrity controls are mandatory. Bypassing them is
strictly forbidden.
...
**Rule**: All point movements must go through the ledger system. Direct balance
...
**Rule**: Only implement behavior explicitly documented in specifications. Do
not invent features.
...
**Rule**: Make the smallest possible changes to achieve the goal. Surgical
edits, not rewrites.
...
```typescript
async function earnPoints(req: any): Promise<any> {
  // ❌ No any types
...
**Rule**: All code must have tests. Financial logic requires comprehensive test
coverage.
...
- ✅ Human review required (see `/docs/governance/COPILOT_GOVERNANCE.md` Section
  2.1)
...
**Rule**: Security is not optional. All code must follow security best
practices.
...
**Rule**: RedRoomRewards is a loyalty platform only. Certain code types are
prohibited.
...
**Rationale**: RedRoomRewards accepts **facts** from external systems (user X
earned Y points), not **logic** (determine if user should earn points).
...
**Rule**: Use the RRR commit prefix enum. Do NOT use feat / fix / docs /
refactor — not valid RRR prefixes.
...
**Rationale**: Financial integrity is critical. AI-assisted code needs human
verification for these sensitive areas.
...
- `/docs/governance/COPILOT_GOVERNANCE.md` - Repository-specific AI development
  rules
...
- **2025-12-15**: Initial version
  - Established core principles (repo as source of truth, no backdoors,
    ledger-first, no invented behavior)
...
**Remember**: These instructions exist to maintain quality, security, and
consistency. Follow them for successful contributions.
